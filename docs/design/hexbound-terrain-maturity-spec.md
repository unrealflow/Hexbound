...
| **B 地标级观感** | 成熟商业 4X/策略场景的地标感：可读的山脉走向与峰岭、崖壁/台地、海岸与浅滩、河谷、森林块。 |
...
- 崖只出现在 `relief`（6 邻 elev 极差）`> CLIFF_DROP` 的边，对标 Humankind「锯齿折线贴合六边、顶底用碎石/缓坡柔化」。
- 斜坡带（`RAMP_DROP…CLIFF_DROP`）顶面 30% 混岩/草（已有 meadow 项 **163–164**，把 `meadow*0.26` 提到 `0.45`，并让它看 `n.y` 不是 `tid`）。
- 侧壁已有 `rockStrata` + `wallLift`（frag **210–225、405–407**）。底边对齐邻格顶后，墙会变矮、被顶面遮挡，不再满屏白裙。
- 台地：只在 `0.32<elev<0.55` 且 `relief` 低时轻微量化；高原顶「中心草、边缘碎石」（Humankind 同心衰减）用 `hexSDF` **反相**（中心草，边缘 15% 岩），不要当描边。
...
**瀑布（P2）：** 河谷边且 `drop>CLIFF_DROP` 时，侧壁 albedo 混白沫 + 向下条带 `abs(sin(wp.x*8.0+fbm))`，底雾 `smoothstep`。不建新 mesh。对标 `civ7-waterfall-cliffs-arid.jpg`：1–3 格跨度的崖 + 水帘。
...
| P1-3 | 崖/台地 | `02`：陡处才有崖，缓处是坡；台地顶可读。对照 `refs/humankind-cliffs-plateaus-rivers.jpg` |
...
| ID | 要求 | 判定 |
|---|---|---|
| P2-1 | 瀑布 | 有河谷切崖处见水帘/沫（允许程序化，不要求每种子都有） |
